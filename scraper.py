...
def raccogli():
    try:
        # Questa versione usa una stringa unica, senza ritorni a capo manuali
        ...
        notizie = {f"news_{i}": a.text.strip() for i, a in enumerate(soup.find_all(class_='card-title', limit=10))}
        
        db.reference('/').set({"notizie": notizie})
